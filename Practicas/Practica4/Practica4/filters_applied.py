...
class appliend_filters:
    def __init__(self, image):
        self.filtros = []
        self.noise_img = cv2.blur(image,(9,9))
        self.peper_noise_img = fl.filters.salt_pepper_noise(image,0.018)
    ...
    def Sobel(self, img, img_name, flag):
        if flag: 
            kernel = fl.filters.sobel_x 
        else: 
            kernel = fl.filters.sobel_y
    ...
    def dervGauss(self, n, img, img_name):
        kernel_bin = fl.filters.gauss_vector(self,n)
        kernel = fl.filters.derv1_gauss(self, n)
        kernel_mat = fl.filters.to_matrix(self,np.transpose(kernel_bin), kernel)
...
if __name__==  "__main__":
    ...
    #imagen with noise
    peper_img = filters_applied.peper_noise_img
